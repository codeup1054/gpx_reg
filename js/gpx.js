        ...
        switch (m.gpxSet) {
            ...
            default:
                ...
                var m_text = $(text_lines).map(function (k, v) {
                    l++;
                    return '<text x="20" y="' + (l * 9) + '" font-family="Arial, sans-serif" fill="%23113388" stroke="none" \
...
function savegpxtoOSMAnd() {
    ...
    $("[type=checkbox]:checked").each(function () {
        ...
        $('.wp_panel table[dataset="' + wpset + '"] tr:not(.header)').each(function (k, v) {
            ...
            t = $(v).children().map(function (ek, ev) {
                return $(ev).text()
            });
...
$(function () {
    ...
    console.log("@@ ",$("#slider_transperency").attr('hist'));
...
function updateIntCols(d) {
    ...
    colors = $(Object.keys(d['colors'])).map(function (k, v) {
...
*/
                ...
                max_set = Math.max.apply(Math, Object.keys(glob_gpx).map(function (o) {
                    return o;
                }))
...
//            console.log("@@ save_gpx_to_json", json_gpx);
        ...
        case "save_google":
            ...
            $(trs).each(function (k, tr) {
                t = $(tr).children("td").map(function (ek, ev) {
                    return $(ev).text()
                }).get();
                vals[row++] = t;
            });
...
*/
    ...
    options = [0, 1].map(function (x) {
        return "<option " + ((v.set_type == x) ? "selected" : "") + ">" + x + "</option>";
    }).join("")
...
function addPoint(set_id, i = {
    name: "Новая точка",  // 2020-02-18 Добавляем новую точку
    description: "Описание",
    lat: "55.4",
    lng: "37.45"
}) {
...
*/
    ...
    function checkKey(e) {
        ...
        switch (e.keyCode) {
            case 13:
            case 37:
                el = el.closest('tr').find('td').eq(index).next();
                break;
            case 39:
                el = el.closest('tr').find('td').eq(index).prev();
                break;
        }
    }
    ...
    $(".datasets td").on("click", function (e) {
        ...
        if (e.ctrlKey) // description column
        {
            ...
            switch (index) {
                case 3:
                    geo_result = geocode(td_val);
                    break;

                case 6:
                    td_val = $(this).parent("tr").find("td:eq(4),td:eq(5)")
                        .map(function () {
                            return $(this).text()
                        }).get();
